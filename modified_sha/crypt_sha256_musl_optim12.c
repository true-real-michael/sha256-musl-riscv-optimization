...
#define ADD64AS32(x, y) ((((x&0xffffffff)+(y&0xffffffff))&0xffffffff)|((x&0xffffffff00000000)+(y&0xffffffff00000000)))
#define ROR17_64(x) (((x >> 17) & 0x00007fff00007fff) | ((x << 15) & ~0x00007fff00007fff)) // 0xffff8000ffff8000
#define ROR19_64(x) (((x >> 19) & 0x00001fff00001fff) | ((x << 13) & ~0x00001fff00001fff))
#define ROR07_64(x) (((x >>  7) & 0x01ffffff01ffffff) | ((x << 25) & ~0x01ffffff01ffffff))
#define ROR18_64(x) (((x >> 18) & 0x00003fff00003fff) | ((x << 14) & ~0x00003fff00003fff))
#define R1_64(x)    (ROR17_64(x) ^ ROR19_64(x) ^ ((x >> 10)        &  0x003fffff003fffff))
#define R0_64(x)    (ROR07_64(x) ^ ROR18_64(x) ^ ((x >>  3)        &  0x1fffffff1fffffff))
#define INDEX64AS32(a, i) (*(uint64_t *)((uint32_t *)a + i))
#define ROR_64(n,k) (((n >> k) & ((0xffffffff >> k) | (0xffffffff >> k << 32))) | ((n << (32-k)) & ((0xffffffff >> k << k) | (0xffffffff >> k << (32 + k)))))
#define T1(i, E, F, G, H, K, W) (H + S1(E) + Ch(E, F, G) + K[i] + W[i])
#define T2(A, B, C) (S0(A) + Maj(A, B, C))
...
static void processblock(struct sha256 *s, const uint8_t *buf) {
    ...
    for (i = 0; i < 8; i++) {
        W64[i] = (uint64_t) buf[8 * i + 4] << 56 |
                 (uint64_t) buf[8 * i + 5] << 48 |
                 (uint64_t) buf[8 * i + 6] << 40 |
                 (uint64_t) buf[8 * i + 7] << 32 |

                 (uint64_t) buf[8 * i + 0] << 24 |
                 (uint64_t) buf[8 * i + 1] << 16 |
                 (uint64_t) buf[8 * i + 2] << 8 |
                 (uint64_t) buf[8 * i + 3];
    }
    for (; i < 32; i++) {
        W64[i] = ADD64AS32(ADD64AS32(R1_64(W64[i - 1]),
                                     W64[i - 8]),
                           ADD64AS32(R0_64(INDEX64AS32(W64, 2 * i - 15)),
                                     INDEX64AS32(W64, 2 * i - 7)));
    }
    ...
    uint32_t c_, d_, g_, h_, t1_;
    for (i = 1; i < 64; i += 2) {
        t1_ = T1(i-1, e, f, g, h, K, W);
        t1 = T1(i, d + t1_, e, f, g, K, W);
        t2 = T2(a, b, c);
        c_ = a;
        d_ = b;
        a = t1 + T2(t1_ + t2, a, b);
